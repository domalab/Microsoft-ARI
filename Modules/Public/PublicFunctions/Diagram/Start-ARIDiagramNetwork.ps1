...
Function Start-ARIDiagramNetwork {
    ...
    ('DrawIONetwork - '+(get-date -Format 'yyyy-MM-dd_HH_mm_ss')+' - Starting Network Diagram Job...') | Out-File -FilePath $LogFile -Append 
        ...
        Function Publish-ARIDiagramStensils {
            ...
            $Script:IconFirewall = "aspect=fixed;html=1;points=[];align=center;image;fontSize=14;image=img/lib/azure2/networking/Firewalls.svg;" # width="71" height="60"
            $Script:IconAppGateway = "aspect=fixed;html=1;points=[];align=center;image;fontSize=14;image=img/lib/azure2/networking/Application_Gateways.svg;" # width="64" height="64"
            ...
            $Script:IconVirtualWAN = "aspect=fixed;html=1;points=[];align=center;image;fontSize=14;image=img/lib/azure2/networking/Virtual_WANs.svg;" # width="65" height="64"
            $Script:IconVirtualHUB = "aspect=fixed;html=1;points=[];align=center;image;fontSize=14;image=img/lib/azure2/networking/Virtual_Network_Hubs.svg;" # width="64" height="64"
            $Script:IconBastion = "aspect=fixed;html=1;points=[];align=center;image;fontSize=14;image=img/lib/azure2/networking/Bastions.svg;" # width="63" height="73"
            ...
            $Script:IconLogicApp = "aspect=fixed;html=1;points=[];align=center;image;fontSize=14;image=img/lib/azure2/integration/Logic_Apps.svg;" # width="64" height="64"
        ...
        try
            {
                ...
                $Script:ManagedID = $Job.ManagedID
                $Script:FrontDoor = $Job.FrontDoor
                $Script:PrivateDNS = $Job.PrivateDNS
                $Script:NetworkWatcher = $Job.NetworkWatcher
                $Script:CDNProfiles = $Job.CDNProfiles
                $Script:DNSZones = $Job.DNSZones
            ...
            $Script:IconManagedID = "aspect=fixed;html=1;points=[];align=center;image;fontSize=14;image=img/lib/azure2/identity/Managed_Identities.svg;" #width="68" height="68"
            $Script:IconFrontDoor = "aspect=fixed;html=1;points=[];align=center;image;fontSize=14;image=img/lib/azure2/networking/Front_Doors.svg;" #width="68" height="68"
            $Script:IconPrivateDNS = "aspect=fixed;html=1;points=[];align=center;image;fontSize=14;image=img/lib/azure2/networking/Private_DNS_Zones.svg;" #width="68" height="68"
            $Script:IconNetworkWatcher = "aspect=fixed;html=1;points=[];align=center;image;fontSize=14;image=img/lib/azure2/networking/Network_Watcher.svg;" #width="68" height="68"
            $Script:IconCDNProfile = "aspect=fixed;html=1;points=[];align=center;image;fontSize=14;image=img/lib/azure2/networking/CDN_Profiles.svg;" #width="68" height="68"
        ...
        try
            {
                ...
                ('DrawIONetwork - '+(get-date -Format 'yyyy-MM-dd_HH_mm_ss')+' - Setting Azure Resources') | Out-File -FilePath $LogFile -Append 
                        ...
                        if($Job.Count -lt 21)
                            { 
                                ...
                                $TempJob = Start-Job -ScriptBlock {
                                    param($subid)
                                    $ERs = @()
                                    $ERs = Get-AzExpressRouteCircuit -SubscriptionId $subid
                                    return $ERs
                                }-ArgumentList $subid
                                $Script:jobs += $TempJob
                                
                                #Getting Virtual Network Gateway
                                $TempJob = Start-Job -ScriptBlock {
                                    param($subid)
                                    $VGWs = @()
                                    $VGWs = Get-AzVirtualNetworkGateway -SubscriptionId $subid
                                    return $VGWs
                                }-ArgumentList $subid
                                $Script:jobs += $TempJob
                                
                                #Getting Virtual Network Gateway Connections
                                $TempJob = Start-Job -ScriptBlock {
                                    param($subid)
                                    $VGWCONs = @()
                                    $VGWCONs = Get-AzVirtualNetworkGatewayConnection -SubscriptionId $subid
                                    return $VGWCONs
                                }-ArgumentList $subid
                                $Script:jobs += $TempJob

                                #Getting Local Network Gateway
                                $TempJob = Start-Job -ScriptBlock {
                                    param($subid)
                                    $LNGs = @()
                                    $LNGs = Get-AzLocalNetworkGateway -SubscriptionId $subid
                                    return $LNGs
                                }-ArgumentList $subid
                                $Script:jobs += $TempJob
                                ...
                                $TempJob = Start-Job -ScriptBlock {
                                    param($subid)
                                    $VWANs = @()
                                    $VWANs = Get-AzVirtualWan -SubscriptionId $subid
                                    return $VWANs
                                }-ArgumentList $subid
                                $Script:jobs += $TempJob
                                ...
                                $TempJob = Start-Job -ScriptBlock {
                                    param($subid)
                                    $VHUBs = @()
                                    $VHUBs = Get-AzVirtualHub -SubscriptionId $subid
                                    return $VHUBs
                                }-ArgumentList $subid
                                $Script:jobs += $TempJob
                                ...
                                $TempJob = Start-Job -ScriptBlock {
                                    param($subid)
                                    $VNETs = @()
                                    $VNETs = Get-AzVirtualNetwork -SubscriptionId $subid
                                    return $VNETs
                                }-ArgumentList $subid
                                $Script:jobs += $TempJob

                                #Getting Kubernetes Service
                                $TempJob = Start-Job -ScriptBlock {
                                    param($subid)
                                    $Aks = @()
                                    $Aks = Get-AzAksCluster -SubscriptionId $subid
                                    return $Aks
                                }-ArgumentList $subid
                                $Script:jobs += $TempJob
                                ...
                                $TempJob = Start-Job -ScriptBlock {
                                    param($subid)
                                    $ANFs = @()
                                    $ANFs = Get-AzNetAppFilesAccount -SubscriptionId $subid
                                    return $ANFs
                                }-ArgumentList $subid
                                $Script:jobs += $TempJob
                                ...
                                $TempJob = Start-Job -ScriptBlock {
                                    param($subid)
                                    $DBRs = @()
                                    $DBRs = Get-AzDatabricksWorkspace -SubscriptionId $subid
                                    return $DBRs
                                }-ArgumentList $subid
                                $Script:jobs += $TempJob

                                #Getting RedHat Open Shift
                                $TempJob = Start-Job -ScriptBlock {
                                    param($subid)
                                    $AROs = @()
                                    $AROs = Get-AzRedHatOpenShiftCluster -SubscriptionId $subid
                                    return $AROs
                                }-ArgumentList $subid
                                $Script:jobs += $TempJob

                                #Getting Azure Functions
                                $TempJob = Start-Job -ScriptBlock {
                                    param($subid)
                                    $FunApps = @()
                                    $FunApps = Get-AzFunctionApp -SubscriptionId $subid
                                    return $FunApps
                                }-ArgumentList $subid
                                $Script:jobs += $TempJob

                                #Getting Azure Containers
                                $TempJob = Start-Job -ScriptBlock {
                                    param($subid)
                                    $Containers = @()
                                    $Containers = Get-AzContainerGroup -SubscriptionId $subid
                                    return $Containers
                                }-ArgumentList $subid
                                $Script:jobs += $TempJob

                                #Getting Azure Kustus
                                $TempJob = Start-Job -ScriptBlock {
                                    param($subid)
                                    $Kustus = @()
                                    $Kustus = Get-AzKustoCluster -SubscriptionId $subid
                                    return $Kustus
                                }-ArgumentList $subid
                                ...
                                $TempJob = Start-Job -ScriptBlock {
                                    param($subid)
                                    $ManagedID = @()
                                    $ManagedID = Get-AzUserAssignedIdentity -SubscriptionId $subid
                                    return $ManagedID
                                }-ArgumentList $subid
                                ...
                                $TempJob = Start-Job -ScriptBlock {
                                    param($subid)
                                    $FrontDoor = @()
                                    $FrontDoor = Get-AzFrontDoor -SubscriptionId $subid
                                    return $FrontDoor
                                }-ArgumentList $subid
                                
                                #Getting Private DNS Zone
                                $TempJob = Start-Job -ScriptBlock {
                                    param($subid)
                                    $PrivateDNS = @()
                                    $PrivateDNS = Get-AzPrivateDnsZone -SubscriptionId $subid
                                    return $PrivateDNS
                                }-ArgumentList $subid
                                
                                #Getting Network Watcher
                                $TempJob = Start-Job -ScriptBlock {
                                    param($subid)
                                    $NetworkWatcher = @()
                                    $NetworkWatcher = Get-AzNetworkWatcher -SubscriptionId $subid
                                    return $NetworkWatcher
                                }-ArgumentList $subid
                                
                                #Getting CDN Profiles
                                $TempJob = Start-Job -ScriptBlock {
                                    param($subid)
                                    $CDNProfiles = @()
                                    $CDNProfiles = Get-AzCdnProfile -SubscriptionId $subid
                                    return $CDNProfiles
                                }-ArgumentList $subid
                                $Script:jobs += $TempJob
                            }
                        ...
                        if($Job.Count -lt 9)
                            { 
                                ...
                                $TempJob = Start-Job -ScriptBlock {
                                    param($subid)
                                    $LBs = @()
                                    $LBs = Get-AzLoadBalancer -SubscriptionId $subid
                                    return $LBs
                                }-ArgumentList $subid
                                $Script:jobs += $TempJob
                                #Getting Public IPs
                                $TempJob = Start-Job -ScriptBlock {
                                    param($subid)
                                    $PIPs = @()
                                    $PIPs = Get-AzPublicIpAddress -SubscriptionId $subid
                                    return $PIPs
                                }-ArgumentList $subid
                                $Script:jobs2 += $TempJob
                                
                                #Getting Network Cards
                                $TempJob = Start-Job -ScriptBlock {
                                    param($subid)
                                    $NICs = @()
                                    $NICs = Get-AzNetworkInterface -SubscriptionId $subid
                                    return $NICs
                                }-ArgumentList $subid
                                $Script:jobs2 += $TempJob
                                
                                #Getting Private Link Services
                                $TempJob = Start-Job -ScriptBlock {
                                    param($subid)
                                    $PLs = @()
                                    $PLs = Get-AzPrivateLinkService -SubscriptionId $subid
                                    return $PLs
                                }-ArgumentList $subid
                                $Script:jobs2 += $TempJob
                                
                                #Getting Bastion Hosts
                                $TempJob = Start-Job -ScriptBlock {
                                    param($subid)
                                    $BHs = @()
                                    $BHs = Get-AzBastion -SubscriptionId $subid
                                    return $BHs
                                }-ArgumentList $subid
                                $Script:jobs2 += $TempJob
                                ...
                                $TempJob = Start-Job -ScriptBlock {
                                    param($subid)
                                    $AGs = @()
                                    $AGs = Get-AzApplicationGateway -SubscriptionId $subid
                                    return $AGs
                                }-ArgumentList $subid
                                $Script:jobs2 += $TempJob
                                ...
                                $TempJob = Start-Job -ScriptBlock {
                                    param($subid)
                                    $FWs = @()
                                    $FWs = Get-AzFirewall -SubscriptionId $subid
                                    return $FWs
                                }-ArgumentList $subid
                                $Script:jobs2 += $TempJob
                                
                                #Getting API Management
                                $TempJob = Start-Job -ScriptBlock {
                                    param($subid)
                                    $APIMs = @()
                                    $APIMs = Get-AzApiManagement -SubscriptionId $subid
                                    return $APIMs
                                }-ArgumentList $subid
                                $Script:jobs2 += $TempJob
                            }
                ...
                #Resource Variables
                if($AZVNETS.count -lt 1 -and !$LimitedOutput){('DrawIONetwork - '+(get-date -Format 'yyyy-MM-dd_HH_mm_ss')+' - WARNING: No Azure Virtual Networks found') | Out-File -FilePath $LogFile -Append}
                if($AZERS.count -lt 1 -and !$LimitedOutput){('DrawIONetwork - '+(get-date -Format 'yyyy-MM-dd_HH_mm_ss')+' - WARNING: No Azure ExpressRoute found') | Out-File -FilePath $LogFile -Append}
                if($AZVGW.count -lt 1 -and !$LimitedOutput){('DrawIONetwork - '+(get-date -Format 'yyyy-MM-dd_HH_mm_ss')+' - WARNING: No Azure Virtual Network Gateway found') | Out-File -FilePath $LogFile -Append}
                
                $Script:ERs = $AZERS
                $Script:VGWs = $AZVGW
                $Script:VGWCONs = $AZVGWCON
                $Script:LNGs = $AZLNG
                $Script:VWANs = $AZVWAN
                $Script:VHUB = $AZVHUB
                $Script:AZVNETs = $AZVNETS
                $Script:AKS = $AZAKs 
                $Script:ANF = $AZANFs
                $Script:DBR = $AZDBRs
                $Script:ARO = $AZAROs 
                $Script:FunApp = $AZFunApps
                $Script:Container = $AZContainers
                $Script:Kusto = $AZKustus
                $Script:LBs = $AZLBs
                $Script:PLs = $AZPLs
                $Script:NICs = $AZNICs
                $Script:PIPs = $AZPIPs
                ...
                $Script:AGs = $AZAGs
                $Script:FWs = $AZFWs
                $Script:APIMs = $AZAPIMs
                
                # Modern networking components
                $Script:ManagedID = $AZManagedID
                $Script:FrontDoor = $AZFrontDoor
                $Script:PrivateDNS = $AZPrivateDNS
                $Script:NetworkWatcher = $AZNetworkWatcher
                $Script:CDNProfiles = $AZCDNProfiles
                $Script:DNSZones = $AZDNSZones
                if($AZVNETS.count -lt 1 -and !$LimitedOutput)
                {
                    ('DrawIONetwork - '+(get-date -Format 'yyyy-MM-dd_HH_mm_ss')+' - WARNING: No Azure Virtual Networks found') | Out-File -FilePath $LogFile -Append
                }
                if($AZERS.count -lt 1 -and !$LimitedOutput)
                {
                    ('DrawIONetwork - '+(get-date -Format 'yyyy-MM-dd_HH_mm_ss')+' - WARNING: No Azure ExpressRoute found') | Out-File -FilePath $LogFile -Append
                }
                if($AZVGW.count -lt 1 -and !$LimitedOutput)
                {
